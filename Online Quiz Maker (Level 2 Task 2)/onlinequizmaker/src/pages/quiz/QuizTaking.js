...
const[answerItems,setAnswerItems]=useState([])
  ...
  return (
    <div className='flex flex-col justify-center items-center px-10 pt-16'>
      ...
      {
        QuizList.quizzes.filter(quiz=>quiz.id===Number(id))
       .map((item)=>(
        <div className='boder-2 boder-red-600'>
{
  item.questions.map((questionItems,index)=>(
...
<input type="radio" className='mr-2' id="choice" name={`question-${questionItems.id}`}
 onChange={()=>handleAnswerSelection(questionItems.id,option)} checked={selectedAnswers[questionItems.id]===option}/>
{option}
      </label>
      // 
    ))}
      ...
      {showModal && (<div className="fixed inset-0 flex items-center justify-center z-50">
          <div className="bg-white w-[30vw] drop-shadow-lg shadow shadow-gray-400 rounded-lg p-10">
            <div className="modal-content">
              ...
              <QuizResult correct={totalScore} incorrect={QuizList.quizzes.length-totalScore}/>
